...
const AuthModal = ({ isOpen, onClose, onDiscord, onGoogle, onAuthSuccess }) => {
  ...
  const handleAuthSuccess = async () => {
    console.log('🔐 AUTH MODAL DEBUG: handleAuthSuccess called');
    // Check if there's a saved essay from landing page
    const savedEssay = localStorage.getItem('landingPageEssay');
    console.log('🔐 AUTH MODAL DEBUG: Saved essay from localStorage:', savedEssay);
    
    if (savedEssay) {
      console.log('📝 AUTH MODAL DEBUG: Found saved essay, processing immediately');
      setIsProcessing(true);
      
      try {
        // Call the custom auth success handler if provided
        if (onAuthSuccess) {
          console.log('📝 AUTH MODAL DEBUG: Calling onAuthSuccess callback');
          await onAuthSuccess();
        } else {
          console.log('📝 AUTH MODAL DEBUG: No onAuthSuccess callback, redirecting to write page');
          // Fallback to old behavior
          window.location.href = '/write';
        }
      } catch (error) {
        console.error('❌ AUTH MODAL DEBUG: Error processing essay after auth:', error);
      ...
      console.log('📝 AUTH MODAL DEBUG: No saved essay, redirecting to dashboard');
            ...
            <div className="px-6 py-5 grid md:grid-cols-2 gap-6 relative">
              ...
              <div className="order-1 md:order-2 space-y-3">
                ...
                <button 
                  ...
                  onClick={async () => { 
                    console.log('🔐 AUTH MODAL DEBUG: Discord button clicked');
                    await onDiscord(); 
                    console.log('🔐 AUTH MODAL DEBUG: Discord auth completed, calling handleAuthSuccess');
                    handleAuthSuccess(); 
                  }}
                >
                ...
                <button 
                  ...
                  disabled={isProcessing}
                  onClick={async () => { 
                    console.log('🔐 AUTH MODAL DEBUG: Google button clicked');
                    await onGoogle(); 
                    console.log('🔐 AUTH MODAL DEBUG: Google auth completed, calling handleAuthSuccess');
                    handleAuthSuccess(); 
                  }}
                >
